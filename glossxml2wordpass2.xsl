...
<!-- Word-compatible XHTML template into which the terms are inserted -->
...
<xsl:variable name="transformationfailed" select="$moodle_labels/data[@name = 'local_glossary_wordimport_transformationfailed']"/>
...
<!-- Place terms in XHTML template body -->
...
<xsl:template match="processing-instruction('replace')[.='insert-meta']">
    ...
    <o:DC.Type><xsl:value-of select="'Glossary'"/></o:DC.Type>
...
<!-- Delete the supplementary paragraphs containing images within each term component, as they are no longer needed -->
