 ...
 * XSLT stylesheet to transform rough XHTML derived from Word 2010 files into a more hierarchical format with divs wrapping each heading and table (glossary term)
 *
...
<xsl:stylesheet
    ...
    <!-- Convert the Heading1 style into a <h1> element (i.e. Category) -->
    ...
    <!-- Convert the Heading2 style into a <h2> element (i.e. term concept), and wrap it and the following table into a div -->
    <xsl:template match="x:p[@class = 'heading2']" priority="2">
        <div class="level2">
            ...
            <!-- Grab the next table following, and put it inside the same div, introducing a simple hierarchy to group the term concept and definition-->
            <xsl:apply-templates select="following::x:table[contains(@class, 'moodleGlossary')][1]" mode="moodleGlossary"/>
        </div>
    </xsl:template>

    <!-- Handle glossary tables in moodleGlossary mode, to wrap them inside a div with the previous heading 2 (concept) -->
    <xsl:template match="x:table[contains(@class, 'moodleGlossary')]" mode="moodleGlossary">
        <xsl:value-of select="$debug_newline"/>
        <table class="moodleGlossary">
            <xsl:apply-templates/>
        </table>
    </xsl:template>

    <!-- Delete glossary tables in normal processing, as they are grabbed by the previous heading 2 style -->
    <xsl:template match="x:table[contains(@class, 'moodleGlossary')]"/>
